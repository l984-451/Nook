// json-edit-fetch-response.js — Edit fetch JSON responses (restricted values, non-trusted).
// Usage: json-edit-fetch-response(editExpression, propsToMatch, [urlPattern])
(function() {
    'use strict';
    ...
    window.fetch = new Proxy(originalFetch, {
        apply(target, thisArg, argumentsList) {
            const input = argumentsList[0]; let url = '';
            ...
            return Reflect.apply(target, thisArg, argumentsList).then(response => {
                ...
                return clone.text().then(text => {
                    try {
                        ...
                        const newHeaders = new Headers(response.headers); newHeaders.delete('content-length');
                        return new Response(JSON.stringify(json), { status: response.status, statusText: response.statusText, headers: newHeaders });
